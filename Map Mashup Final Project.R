...
#Packages for map analysis
library(ggplot2)
library(ggmap)
library(tidyr)
library(stringr)

#Further cleaning to separate coordinate column, remove na's and brackets
...
# Create US, California, and New York maps
us <- map_data("state")
...
usairlinemap <- USmap + geom_point(data = tweets, aes(x = tweets$Lon, y = tweets$Lat, color = airline))
usairlinemap

ussentimentmap <- USmap + geom_point(data = tweets, aes(x = tweets$Lon, y = tweets$Lat, color = airline_sentiment))
ussentimentmap
...
newyorktweetmap <- newyorkmap + geom_point(data = tweets, aes(x = Lon, y = Lat), alpha = 0.1)
newyorktweetmap
